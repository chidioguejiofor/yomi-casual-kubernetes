...
import { cache, uuidGenerator } from "../services/cache";
import {
  LOGIN_TOKEN_AGE,
...
`;
  ...
  public sendVerificationEmail = async (
    ...
    const registerId = cache.cacheVerificationEmail(
      type,
      {
        email,
        redirectURL,
  ...
  private handleLoginResponse = (
    ...
    cache.cacheData(loginKey, token, LOGIN_TOKEN_AGE);
  ...
  public async confirmEmail(req: RequestType, res: Response): Promise<unknown> {
    ...
    const redisData = await cache.getCachedRegisterEmail(
      "REGISTER",
      confirmEmailId
    );
  ...
  loggedInUserChangesPassword = async (req: RequestType, res: Response) => {
    ...
    cache.deleteByPattern(userLoginKeys);
